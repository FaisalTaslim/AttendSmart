...
const contentsContainer2 =
  document.getElementsByClassName("dashboard-contents")[0];
...
async function startSessionAndRedirect(override = false) {
  ...
  const sessionRes = await fetch(
    `/dashboard/admin/start-employee-session?${params.toString()}`,
  );
...
getBtn.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (data.status === "active") {
      ...
      alert(
        `No active session available. Check back during your shift window.`,
      );
    }
  } catch (err) {
    console.error(err);
